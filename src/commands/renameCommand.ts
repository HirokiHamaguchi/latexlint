...
async function handleLabelRename(doc: vscode.TextDocument, res: LabelResult) {
    ...
    const occurrenceMessage = `"${currentLabel}" to "${newLabel}" (${matches.length} occurrence${matches.length !== 1 ? 's' : ''})`;
    ...
    const shouldApply = await showDiffAndConfirm(
        doc.uri,
        modifiedText,
        `Label Rename: ${currentLabel} → ${newLabel}`
    );
...
export default async function renameCommand() {
    ...
    if (resLabel.result !== undefined)
        return await handleLabelRename(doc, resLabel.result);
    else if (resLabel.message)
        return vscode.window.showErrorMessage(resLabel.message);

    const resBE = findBeginEndTargets(text, cursorOffset);
    if (resBE.result !== undefined)
        return await handleBeginEndRename(doc, editor, resBE.result);
    else if (resBE.message)
        return vscode.window.showErrorMessage(resBE.message);
