...
export default async function selectRules() {
    ...
    quickPick.selectedItems = quickPick.items.filter(item => !disabledRules.includes(item.label));
    ...
    quickPick.onDidAccept(async () => {
        ...
        await config.update('disabledRules', unselectedItems, vscode.ConfigurationTarget.Workspace);
