export function computeValidRanges(text: string, languageId: string): [number, number][] {
    ...
    if (languageId === 'latex')
        ...
        console.log(`Unknown languageId ${languageId} in computeValidRanges.`);
    const mergedInvalidRanges = mergeRanges(invalidRanges);
    return invertRanges(mergedInvalidRanges, text.length);
}

export function isPositionValid(idx: number, validRanges: [number, number][]): boolean {
    ...
    for (const env of verbatimEnvs) {
        ...
        while ((beginMatch = beginRegex.exec(text)) !== null) {
            ...
            if (endMatch)
                ...
                ranges.push([beginMatch.index, text.length]);
        }
    }
...
function mergeRanges(ranges: [number, number][]): [number, number][] {
    ...
    const sorted = ranges.sort((a, b) => { return a[0] !== b[0] ? a[0] - b[0] : a[1] - b[1]; });
...
function invertRanges(invalidRanges: [number, number][], textLength: number): [number, number][] {
